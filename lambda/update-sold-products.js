...
const getAndUpdateProduct = async (uid, size, quantity , Model) => {
  let product = await Model.findOne({uid: uid, size: size})
  if(product){
    product = await Model.updateOne({uid: uid, size: size}, {sold: product.sold + Number(quantity)})
  }else{
    product = new Model({uid, size, sold: quantity})
    product.save()
  }
