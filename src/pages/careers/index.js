...
const Careers = () => {
    ...
    const data = useStaticQuery(graphql`
        query CareersPage{
            page:markdownRemark(frontmatter:{templateKey:{eq: "careers-page"}}){
                frontmatter{
                    heroImage{
                        childImageSharp{
        ...
        <Layout fullWidth={true}>
            <main className={styles.fullWidth}>
                <div className={styles.hero}>
                    <Image className={styles.heroImage} fluid={data.page.frontmatter.heroImage.childImageSharp.fluid} alt=''/>
                </div>
                ...
                <div className={styles.logos}>
                    <div className={styles.mainLogo}>
                        {data.logos.edges.map(({node:logo}) => {
                            if(logo.frontmatter.title === 'Networks'){
                                return <Image fluid={logo.frontmatter.logoImage.childImageSharp.fluid} alt=''/>
                            }
                        })}
                    </div>
                    <div className={styles.otherLogos}>
                        {data.logos.edges.map(({node:logo}) => {
                            if(logo.frontmatter.title !== 'Networks'){
                                return <Image style={{width:`${100 / data.logos.edges.length - 2}%`}} className={styles.otherLogosItem} fluid={logo.frontmatter.logoImage.childImageSharp.fluid} alt=''/>
                            }
                        })}
                    </div>
                    <hr/>
                ...
                <div className={styles.kickerText}>
                    ...
                    <p>RUNBGD Networks champions the people, brands and new trends you need to know now and will obsess over next</p>
                </div>
                <div className={styles.sliderContainer}>
                    <FullWidthSlider>
                        {data.page.frontmatter.slides.map(slide => {
                            return <SwiperSlide className={styles.sliderSlide}>
