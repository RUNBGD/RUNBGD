    ...
    return(
        <div className={styles.locationCards}>
            {locations.map(({node:location}, index) => {
                ...
                if(yCoord){
                    distanceFromStart = distance(xCoord, yCoord, location.frontmatter.longitude, location.frontmatter.latitude, "K")
                }else{
                    distanceFromStart = 0;
                }
                ...
                return <div className={styles.locationCard} style={{order:Number(distanceFromStart).toFixed(0)}} key={index} onClick={() => {setCurrentX(0);setCurrentY(0);setTimeout(()=>{setCurrentX(location.frontmatter.longitude);setCurrentY(location.frontmatter.latitude)}, 100)}}>
                    ...
                    <div className={styles.cardText}>
                        ...
                        {yCoord && <p className={styles.distance}>{distanceFromStart} km</p>}
                    </div>
                </div>
                }
            })}
