...
const ShopProduct = ({images, title, price, availableSizes}) => {
    ...
    const [currentImage, setCurrentImage] = useState(images[1] && images[1].image.childImageSharp.fluid)
            ...
            <div className={`${styles.miscInformation} ${hovered && styles.visible}`}>
                ...
                <div className={styles.thumbnailsContainer}>
                    {images.map((image, index) => {
                        if(index < 4){
                            return <div className={styles.thumbnail} onMouseEnter={() => setCurrentImage(images[index].image.childImageSharp.fluid)}>
